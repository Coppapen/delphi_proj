...
begin
  HistoryMemo.Lines.Add(FormatDateTime('yy/mm/dd hh:nn:ss', Now) + #9 + Text);
end;
...
begin
  ...
  GetFocusedFrame.CopyOrMoveFiles(Dest);
...
begin
  ...
  pb.Left := 2;
  pb.Top := 2;
  pb.Height := StatusBar.Height - 4;
  pb.Width := StatusBar.Panels[0].Width - 4;
  pb.Min := 0;
  pb.Max := 100;
end;
...
begin
  ...
  StatusBar.Top := HistoryMemo.Top + 1;
end;
...
begin
  ...
  try
    ...
    HistoryMemo.Font.Name := AppSettings.HistoryMemoConfig.Font.Name;
    HistoryMemo.Font.Size := AppSettings.HistoryMemoConfig.Font.Size;
    HistoryMemo.Font.Color := AppSettings.HistoryMemoConfig.Colors.Text;
    HistoryMemo.Color := AppSettings.HistoryMemoConfig.Colors.Background;
  finally
    AppSettings.Free;
  end;
end;
...
begin
  ...
  Frame2.CopyOrMoveFiles(Dest, True);
  Frame1.ChangeDirectory(Frame1.CurrentDirectory);
  Frame2.ChangeDirectory(Frame2.CurrentDirectory);
end;

procedure TMainForm.MoveWindow;
begin
  SendMessage(MainForm.Handle, WM_SYSCOMMAND, SC_MOVE, 0);
end;
...
procedure TMainForm.ReloadUnfocusedFrame;
begin
  GetUnfocusedFrame.Reload;
end;

procedure TMainForm.ResizeWindow;
begin
  SendMessage(MainForm.Handle, WM_SYSCOMMAND, SC_SIZE, 0);
end;
...
begin
  if IsForward then
    HistoryMemo.Perform(WM_VSCROLL, SB_PAGEDOWN, 0)
  else
    HistoryMemo.Perform(WM_VSCROLL, SB_PAGEUP, 0);
end;
...
begin
  ...
  begin
    ...
    HistoryMemo.Lines.Add(StringOfChar('-', 50));
    HistoryMemo.Lines.Add('ファイル名   :' + ExtractFileName(FileName));
    HistoryMemo.Lines.Add('場所         :' + ExtractFilePath(FileName));
    with FileInfo do
    begin
      if TDirectory.Exists(FileName) then
      begin
        HistoryMemo.Lines.Add('ファイル数   :' + FileCount.ToString);
        HistoryMemo.Lines.Add('フォルダ数   :' + FolderCount.ToString);
      end;
      HistoryMemo.Lines.Add('サイズ       :' + FormatFloat('#,##0', FileSize) + ' Byte ' +
        FormatFileSize(FileSize));
      HistoryMemo.Lines.Add('作成日時     :' + FormatDateTime('yyyy/mm/dd hh:nn:ss', CreationTime));
      HistoryMemo.Lines.Add('更新日時     :' + FormatDateTime('yyyy/mm/dd hh:nn:ss', LastWriteTime));
      HistoryMemo.Lines.Add('アクセス日時 :' + FormatDateTime('yyyy/mm/dd hh:nn:ss', LastAccessTime));
    end;
  end;
end;
