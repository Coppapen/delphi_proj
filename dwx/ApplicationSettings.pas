...
uses
  ...
  System.JSON, System.JSON.Types, System.Classes, System.IOUtils;
...
type
  ...
  public
    ...
    property TabSetConfig: TTabSetConfig read FTabSetConfig write FTabSetConfig;
...
begin
  ...
  try
    ...
    if not TDirectory.Exists(FCurrentDirs.L) then
      FCurrentDirs.L := ExtractFilePath(Application.ExeName);
    if not TDirectory.Exists(FCurrentDirs.R) then
      FCurrentDirs.R := ExtractFilePath(Application.ExeName);
    FCurrentDirs.L := ExcludeTrailingPathDelimiter(FCurrentDirs.L);
    FCurrentDirs.R := ExcludeTrailingPathDelimiter(FCurrentDirs.R);
    FWindowConfig := Config.WindowConfig;
    FTabSetConfig := Config.TabSetConfig;
...
begin
  ...
  try
    ...
    Config.TabSetConfig := FTabSetConfig;
