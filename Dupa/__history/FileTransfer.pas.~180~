...
type
  ...
  TDeleteProgressEvent = procedure(Sender: TObject; FileName: string;
    Percent: Integer) of object;
...
begin
  ...
  begin
    ...
    Percent := Round(FDeletedFileCount * 100 / FTotalFileCount);
    DoProgress(FileName, Percent);
  end;
...
procedure TDeleteWorker.DoProgress(FileName: string; Percent: Integer);
begin
  if Assigned(FOnProgress) then
  begin
    Synchronize(procedure
...
procedure TDeleteWorker.TotalDeleteFilesCount(const Path: string);
var
  FileList: TArray<string>;
...
begin
  ...
  begin
    FileList := TDirectory.GetFiles(Path, '*', TSearchOption.soAllDirectories);
    Inc(FTotalFileCount, Length(FileList));
  end else
  if FileExists(Path) then
    Inc(FTotalFileCount);
end;
