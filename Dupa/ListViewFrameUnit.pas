...
type
  ...
  private
    ...
    procedure CloseTab;
    procedure CopyDirectoryPathToClipboard;
...
begin
  ...
  try
    ...
    TextHeight := TempCanvas.Canvas.TextHeight('あ');
...
begin
  ...
  if Filter <> '' then
    StatusPanel.Caption := StatusPanel.Caption + ' ( フィルタリング中 )';
...
begin
  ...
  if TotalWidth > lv.Columns[1].Width then
    lv.Columns[0].Width := TotalWidth - lv.Columns[1].Width - 10;
end;

procedure TListViewFrame.CopyDirectoryPathToClipboard;
begin
  Clipboard.AsText := FCurrentDirectory;
end;

procedure TListViewFrame.CopyFileNameToClipboard;
var
  List: TStringList;
  i: Integer;
...
begin
  ...
  begin
    Clipboard.AsText := ExtractFileName(CurrentLineItemPath);
    MainForm.AddOperationHistory(
      Format('パスをクリップボードにコピーしました [ %s ]', [CurrentLineItemPath]));
  end else
  begin
    List := TStringList.Create;
    try
      for i := 0 to FSelectedItems.Count - 1 do
        List.Add(ExtractFileName(FSelectedItems[i].FileName));
      Clipboard.AsText := List.Text;
    finally
      List.Free;
    end;
    MainForm.AddOperationHistory('パスをクリップボードにコピーしました');
  end;
end;
...
procedure TListViewFrame.CopyPathToClipboard;
var
  List: TStringList;
  i: Integer;
...
begin
  if FSelectedItems.Count = 0 then
  begin
    Clipboard.AsText := CurrentLineItemPath;
    MainForm.AddOperationHistory(
      Format('パスをクリップボードにコピーしました [ %s ]', [CurrentLineItemPath]));
  end else
  begin
    ...
    try
      for i := 0 to FSelectedItems.Count - 1 do
        List.Add(FSelectedItems[i].FileName);
      Clipboard.AsText := List.Text;
    finally
      List.Free;
    end;
...
begin
  ...
  try
    ...
    FileImageList.Height := TempCanvas.Canvas.TextHeight('あ') + 6;
  finally
    TempCanvas.Free;
  end;
...
begin
  ...
  begin
    ...
    lv := FileListView;
    if lv.Canvas.TextWidth(FileName) > lv.Columns[0].Width - ColumnWidthMargin then
    begin
...
begin
  ...
  begin
    ...
    Path := ExtractFilePath(NewFilePath);
    FileName := ExtractFileName(NewFilePath);
    MainForm.AddOperationHistory('指定したファイル名は既に存在するので変更しました');
  end;
  MainForm.AddOperationHistory(
    Format('新規ファイルを作成しました [ %s ]', [NewFilePath]));
end;
...
begin
  ...
  begin
    Reload;
    Index := IndexOfFileName(DirName + NewName);
    FileListView.ItemIndex := Index;
    ScrollToItem(Index);
