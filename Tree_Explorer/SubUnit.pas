...
var
  ...
  Win323FindData: TWin32FindDataW;
  Buffer: PChar;
...
begin
  ...
  begin
    ...
    Buffer := AllocMem((MAX_PATH + 1) * 2);
    try
      ShellLink.GetPath(Buffer, MAX_PATH, Win323FindData, SLGP_UNCPRIORITY);
  ...
  begin
    ...
    begin
      ...
      begin
        if (TDirectory.Exists(NameSpace.NameParseAddress)) then
        begin
          Path := NameSpace.NameParseAddress;
          if (LowerCase(ExtractFileExt(Path)) = '.lnk') then
            Path := LnkFileFromPath(Path);
