...
begin
  ...
  FMenuInitialItemCount := PopupMenu1.Items.Count;
end;
...
const
  FixedItemsCaption: array [0..1] of string = (
    '新しいウィンドウを開く',
    'アプリケーションを終了する'
  );
...
begin
  ...
  for var i := Low(FixedItemsCaption) to High(FixedItemsCaption) do
  begin
    var NewItem := TMenuItem.Create(Self);
    NewItem.Caption := FixedItemsCaption[i];
    case i of
      0: NewItem.OnClick := N1Click;
      1: NewItem.OnClick := N2Click;
    end;
    PopupMenu1.Items.Add(NewItem);
  end;
end;
